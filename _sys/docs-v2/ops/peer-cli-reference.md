...
- **Session reuse status:** ag IPC (via winpty) runs fine, but reuse still does not
  restore context because agy assigns its **own** id and ignores the injected uuid.
  Reuse IS implementable in principle (capture agy's real id → resume by it); the
  earlier "not practical" verdict was based on the console artifact and is retracted.
...
| ag | `agy --conversation <agy-own-id>` | agy -p runs (via winpty/console); reuse blocked — agy owns the id, ignores injected uuid | ⚠️ implementable if we capture agy's real id (revisit) |
