...
> **PRO-19 implementation status (ENFORCED — 2026-06-25).** The three intended controls are now active in `hub.py._guard_action()`: (1) `HUB_ORIGIN=terminal` callers are blocked from all `mutating_hub_actions` (PRO-19/C1); (2) `decision_tier_floor` requires at least `effort` tier for governance mutations (PRO-19/C2); (3) all mutating transport paths are programmatically guarded, with system-automated actions (sweeps, health checks, session lifecycle) explicitly exempted (PRO-19/C3). The `--force-tier0` flag provides the mandatory [ESCALATE] path for Tier-0 human authority (INV-03). Consensus also gained `supermajority` outcome support: when all healthy voters agree and quarantined voters are auto-abstained, rounds finalize without escalation. Derived from the "Resetting blocked consensus and taking over" incident.
