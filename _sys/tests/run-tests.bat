...
:: run-tests [--unit-consensus] 신규: N-Node 합의 심화 테스트
...
:run_unit_core
echo [tests] --- Unit Core (test_hub.py, test_hub_consensus.py) ---
python -m pytest "%~dp0unit\test_hub.py" "%~dp0unit\test_hub_consensus.py" -v --tb=short
if errorlevel 1 set "_FAIL=1"
if "%_MODE%"=="unit" goto :done
goto :run_unit_consensus

:run_unit_consensus
:: Legacy label kept for flow, now combined with core
if "%_MODE%"=="unit-consensus" goto :done
goto :run_unit_edge
...
:run_integration
echo [tests] --- Integration and Scenarios (Python Refactored) ---
