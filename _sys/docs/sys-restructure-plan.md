...
> Status: DEBATE_FINAL_R4 — gc+cx 4-round 끝장 교차검토 완료 (2026-06-15)
> Date: 2026-06-14 (updated 2026-06-15 R4)
> Author: cc (coordinator), gc (cross-reviewer R1+R3+R4), cx (independent auditor R2+R3)
> Protocol: R:10 (Brain Sync) — 전체 피어 끝장 플랜
> Review: R4: Root Swap ADOPTED(디스크 충분), IPC peers/{id}/ipc/ 설계, 글루파일 slim화
> ⚠️ FINAL: ROOT SWAP ADOPTED, IPC/글루파일 신규 설계, verify-all V1~V15, EXECUTION READY
...
> - `infra.json` = 모든 경로 (`base_dirs`, `path_entries`, `bat_locations`, `config_registry`, `tool_paths`) + **IPC 경로 맵핑** (`ipc_paths` per-peer) — 단일 경로 진실 소스
>
> **infra.json ipc_paths 예시 (R4 신규):**
> ```jsonc
> "ipc_paths": {
...
| `_sys/gemini/{peer}-*.txt` (IPC 파일 하드코딩) | `infra.json["ipc_paths"]["{peer_id}_inbox"]` |
...
5. runtime/recovery/ 생성 + 복구 스크립트 작성 (cx: 영구 홈):
   a) _sys/runtime/recovery/rollback.ps1:
      # cut-over 역전: _sys 삭제 → _sys_old_rollback_* 를 _sys로 rename
      #   → virtualizer.py mount (원본 junction 재등록) → pytest 통과 확인
...
| MEDIUM | IPC inbox 파일 누적 (처리 안 된 쿼리) | 디스크 스페이스 + 혼란 | hub.py capture 후 archive_on_capture 자동 이동; V14 커버리지 체크 |
| MEDIUM | 글루파일(stub) ↔ canonical 문서 drift | AI 피어에 오래된 지침 노출 | hub.py sync-glue --all 로 재생성; verify-all V13 sync check |
...
| P4 | hub.py IPC 쿼리 경로: infra.json["ipc_paths"]["{peer}_inbox"] 기반으로 변경 | R4 |
...
8. IPC 쿼리 파일 경로 변경 — hub.py 호환성?
   → ✅ 설계: infra.json["ipc_paths"]["{peer}_inbox"] 기반 동적 해석. 구 _sys/gemini/ 하드코딩 제거.
