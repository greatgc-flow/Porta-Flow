...
> Status: DEBATE_FINAL_R5 — gc+cx 5-round 끝장 교차검토 완료 (2026-06-15)
> Date: 2026-06-14 (updated 2026-06-15 R5)
> Author: cc (coordinator), gc (cross-reviewer R1+R3+R4+R5), cx (independent auditor R2+R3+R5)
> Protocol: R:10 (Brain Sync) — 전체 피어 끝장 플랜
> Review: R4: Root Swap ADOPTED, IPC 설계, 글루파일 slim화 | R5: MECE 통합 (pathmap 제어 평면, managed-links, IPC 실패 라이프사이클, glue sync 트리거, 엣지케이스 §11)
> ⚠️ FINAL: ROOT SWAP ADOPTED, MECE INVARIANTS 이행 (I-03~I-05/I-09), EXECUTION READY
...
**virtualizer.py 의무 시맨틱 (pathmap 내재화 — R5):**
```
...
신규: virtualizer.py → pathmap 시맨틱 내재화
  ...
  apply:     lock 획득 → 정션 생성 → managed-links.json 원자적 갱신 → audit → lock 해제
  status:    managed-links.json + FS 실제 상태 비교 출력
  prune:     managed orphan 탐지 (registry에 있고 path-map.json에 없음)
  prune --apply: 물리 삭제 성공 확인 후 registry 제거 (Ghost Junction 방지)
```
...
```
T1. hub.py 시작 시 (eager) — canonical 문서의 mtime/hash를 stub과 비교
    → 변경 감지 시 자동 재생성
T2. action_ask 직전 (lazy) — hash drift 감지 시 경고 + 재생성 후 진행
...
8b. ⚠️ pathmap.lock 초기 생성 (Build Phase 시작 선언 — MECE I-04):
    New-Item -ItemType File "_sys_new\data\state\pathmap\pathmap.lock" -Force
    # lock 파일: {"pid": $PID, "acquired_at": "...", "operation": "root_swap_build"} 기록

8c. 📋 SHA256 manifest 생성 (_sys_new/ 완성 후 적용 — MECE Copy-Verify-Delete 토큰):
    # env/, tools/ 정션 대상 + ipc/, data/temp/ 제외 → 텍스트 설정 파일만 대상
    # Root Swap cutover.ps1 실행 시 manifest로 _sys_new/ 무결성 검증 후 진행
    # (Phase 0 단계 11 완료 후 실행 — V1~V10 PASS 직전)
    python _sys_new/core/hub.py verify-manifest --sys-dir _sys_new --create \
      --exclude env tools data/temp peers/*/ipc \
      --output _sys_new/data/state/pathmap/manifest.sha256
    # manifest 존재 → cutover.ps1이 소비하는 마이그레이션 토큰
...
12. Validate PASS → cutover.ps1 실행 준비 완료 메시지 출력
