...
### Universal · Workspace-Independent · Multi-Vendor · Symmetric Participation · Functionally Complete · Quality-Attribute Governed

> **Version**: 8.0 | **Date**: 2026-06-10
> **Review History**: v3 (base) → v4 (Human-in-Loop) → v5 (Universal) → v6 (Multi-vendor+Participation) → v7 (Complete) → v8 (Flexibility+Scalability+Maintainability)
> **Collaboration**: Multi-node (CC+GC+Human) | R=10 Unanimous consensus, 2-round audit
> **Supersedes**: TAXONOMY_v7.md (READ-ONLY, path: `P:\_sys\docs\TAXONOMY_v7.md`)
> **New in v8**: §1.5 Quality Attributes Overlay; items 0-5, 2-8-5, 3-13, 3-14, 4-10, 4-11, 5-10, 5-11; T25–T27; C10–C11; 6 new parameters; G39–G42; total 58 items, 36 params.
...
| §3 | Full MECE taxonomy tree (58 items) |
| §4 | Binary/deterministic KPI matrix |
| §5 | Architectural trade-offs (T1–T27) |
| §5.5 | Logical contradictions (C1–C11) |
| §6 | Governance parameter catalogue (36 params, 38 total keys) |
| §7 | Maturity scoring model |
| §8 | Gap analysis log (G1–G42) |
...
| **Kaizen** | Continuous Process Remediation — iterative improvement by identifying and fixing root causes of failures |
| **5-Why** | Root cause analysis technique: ask "Why?" five times in succession to reach the fundamental cause |
| **Primary Success Scenario** | The primary designed use case of a deliverable, verified end-to-end |
| **Gini Coefficient** | Statistical measure of distribution inequality: 0 = perfectly equal, 1 = single-node monopoly |
...
| **Quality Attribute** | A cross-cutting non-functional property describing HOW the governance system behaves: Flexibility, Scalability, or Maintainability. Distinct from functional governance categories (Cat 0–6). |
...
This section defines three cross-cutting quality attributes that must be MECE-ly covered across the
governance framework. Quality attributes describe **HOW** the governance system performs, not WHAT
it governs. They are measured via dedicated items within their respective lifecycle categories
(Cat 0–6), preserving the closed-loop structure rather than creating separate categories.

#### Attribute Definitions

| Attribute | Symbol | Definition | Scope |
|:----------|:------:|:-----------|:------|
| **Flexibility** | [FLEX] | Ability to adapt governance behavior without breaking correctness. Examples: adding a new AI vendor without restructuring governance; changing parameters mid-session; hot-reloading config without node restart; swapping a vendor instance during execution. | Structural adaptability + runtime reconfigurability |
| **Scalability** | [SCALE] | Ability to maintain governance quality as the system grows in any dimension: (a) horizontal — more nodes; (b) vertical — more complex tasks; (c) temporal — longer sessions; (d) artifact scale — larger repositories. | Quantitative growth without governance degradation |
| **Maintainability** | [MAINT] | Ability to evolve the governance system sustainably over time. Examples: detecting configuration drift before it causes failures; measuring governance debt; keeping governance documents fresh relative to implementation. | Long-term evolvability + operational health |

#### v7 Coverage Analysis and v8 Gap Closure

| Attribute | v7 Existing Coverage | v7 Gap | v8 New Items | Coverage After v8 |
|:----------|:--------------------|:-------|:-------------|:------------------|
| FLEX | 4-3 (IPC abstraction), 4-9 (vendor adapter), 2-6 (participation), 2-8 (dynamic roles), 5-8 (degraded mode), T22 (~62%) | Runtime reconfigurability without restart; non-functional constraint capture; mid-session vendor hot-swap | 0-5, 2-8-5, 5-10 | ~100% |
| SCALE | 5-7 (concurrent sessions), 2-7 (sub-teaming), 5-6 (quota), T11 (~57%) | T11 has no measurable KPI; artifact-scale governance; governance overhead bounding | 4-10, 5-11 + T11 KPI in §4 | ~100% |
| MAINT | 3-3 (change mgmt), 3-8 (kaizen), 4-4 (version mgmt), 5-9 (velocity), 1-4 (instruction efficacy) (~63%) | Governance debt measurement; configuration drift detection; document freshness vs. implementation | 3-13, 3-14, 4-11 | ~100% |
| **Overall** | **~60% MECE coverage** | | | **~100% MECE coverage** |

> Coverage percentages = FRAMEWORK-level definition completeness (are the governance rules written?),
> not IMPLEMENTATION completeness (has the rule been coded?).
> Implementation state is tracked in §8 (G39–G42) and Appendix A.4.

#### Quality Attribute Closed-Loop Cycle

Quality attributes span all lifecycle phases. The Human-first entry point is **0-5**, where the
Human declares flexibility, scalability, and maintainability constraints before infrastructure is
provisioned (Cat 4):

```
[Human] 0-5: Declare F/S/M constraints
  → Cat 4: Provision environment respecting constraints (4-10, 4-11)
  → Cat 5: Enforce constraints at runtime (5-10, 5-11)
  → Cat 3: Measure and correct violations (3-13, 3-14)
  → Cat 6: Deliver + feed F/S/M lessons to next cycle
  → Cat 0: Refine constraints for next session  [↩ closed loop]
```

#### Completeness Measurement (Quality Attributes)

For each attribute, completeness = (items with T≥T2 implementation) / (total items tagged with attribute) × 100%.
Measured by script: count items in §4 KPI matrix tagged [FLEX]/[SCALE]/[MAINT] with PASS status.
...
|:-----------|:---------------|:-------------|
| Cat 0 → Cat 4 | Intent schema (Goal ID, Scope, Constraints, Success Criteria, Delivery Format, **F/S/M constraints**) | — |
| Cat 4 → Cat 1 | Verified environment manifest (adapters ready, node profiles loaded, **scale mode set**) | Bootstrap fail → Human Gate → fix → retry Cat 4 |
...
| Cat 3 → Cat 5 | Validated artifact + integrity report (schema OK, tests pass, risks cleared, **GD Score updated**) | Validation fail → reject → back to Cat 2 |
| Cat 5 → Cat 6 | Governed artifact + health metrics (within budget, no critical alerts, **overhead ratio OK**) | Budget exhausted → HALT + escalate |
| Cat 6 → Cat 0 | Delivery ACK + lessons learned + next-session seed + **F/S/M constraint refinements** | Human rejects → revise → back to Cat 2 |
...
│      Human MUST declare quality attribute constraints (flexibility, scalability,
│      maintainability) before infrastructure is provisioned (Cat 4). These constraints
│      scope all subsequent governance decisions.
│      This is the Human-first entry point for the Quality Attribute closed-loop cycle (§1.5).
│      Example: Human declares "Must support 5,000+ files without governance slowdown" (SCALE),
│      "Allow parameter hot-reload mid-session" (FLEX), "Governance docs reviewed every 30 days" (MAINT).
│      ├── 0-5-1: Scalability constraints — max artifact count, max node count, max session duration
│      ├── 0-5-2: Flexibility constraints — hot-reload permitted (Y/N), vendor swap permitted (Y/N)
...
│  │   └── 4-3-5: Dynamic transport scaling — IPC transport layer MUST support runtime
│  │               capacity adjustment without node restart; throughput scales with
│  │               active node count without protocol changes
│  │
...
│  │   When a session's artifact count exceeds `artifact_scale_threshold`, full-repository
│  │   policy gates (3-2-1) and blast-radius analysis (3-3-4) MUST apply localized
│  │   dependency scope rather than full-repository scope. This prevents governance
│  │   mechanisms from becoming bottlenecks as repositories grow.
│  │   Example: Repository with 8,000 files (above threshold 5,000) → blast-radius analysis
│  │   bounded to the affected module's dependency subgraph, not all 8,000 files.
│  │   ├── 4-10-1: Scale trigger detection — automated count of artifacts in session scope;
│  │   │           triggers "localized mode" when count > `artifact_scale_threshold`
│  │   ├── 4-10-2: Localized dependency scope — blast-radius bounded to affected subgraph
│  │   │           (direct dependencies + 1 level of transitive dependencies)
│  │   └── 4-10-3: Full-scope override — human may explicitly request full-scope analysis;
│  │               override logged; `artifact_scale_threshold` temporarily raised for session
│  │
│  └── 4-11: Governance Document Freshness  [C|T3]  [MAINT]
│      Governance documents MUST be reviewed against their corresponding implementation
│      at a defined cadence. A versioned document (4-4) may still contain stale content if
│      it describes features that no longer exist or misses features that do exist.
│      Example: PROTOCOL.md documents a "3-strike halt" but implementation uses "2-strike halt"
│      — the document is versioned but stale.
│      ├── 4-11-1: Review schedule — all governance docs reviewed at `doc_freshness_interval`
│      │           days; last-review timestamp tracked per document
│      ├── 4-11-2: Orphaned reference detection — references to undefined components,
│      │           deprecated APIs, or removed parameters flagged automatically
│      └── 4-11-3: Staleness alert — any doc with no review event in
│                  > `doc_freshness_interval` days → WARN escalation
│
...
│  │   └── 2-8-5: Mid-session vendor/instance hot-swap — when a new vendor capability is
│  │               required during execution, the replacing node MUST:
│  │               (1) complete all held directives or hand off via 2-6-2 before exit;
│  │               (2) have a registered adapter (4-9) ready before activation;
│  │               (3) re-sync working memory state (1-2-3) before accepting new directives.
│  │               Hot-swap event logged to 3-9 as explicit state-machine boundary.
│  │
...
│  external invocation governance, governance debt, configuration drift.
...
│  │   The system MUST continuously measure cumulative governance shortcuts and deferred actions.
│  │   Governance Debt Score (GD) = (active_bypass_count × 5) + (stale_pm_count × 3) +
│  │   (floor(degraded_hours/24) × 2). Unit: GD points. Ceiling: `governance_debt_ceiling`.
│  │   Example: 2 active bypasses + 1 stale PM + 48 degraded hours =
│  │   (2×5) + (1×3) + (floor(48/24)×2) = 10 + 3 + 4 = 17 GD points.
│  │   ├── 3-13-1: Bypass accumulation tracking — count of active policy bypasses (3-10-3)
│  │   ├── 3-13-2: Stale post-mortem count — PMs open > 48 hours (from 3-8) per period
│  │   ├── 3-13-3: Degraded-mode hours — cumulative hours in degraded mode (from 5-8) per period
...
│      When governance parameters have both code-level defaults AND external config file values,
│      the two MUST remain synchronized. Drift = any parameter where code default ≠ config value.
│      Example: code has HANDOFF_MAX_CHARS=12000 but hub_config.json says 8000 →
│      this is drift → startup MUST fail with a diff report identifying the mismatch.
│      Boundary note: 3-14 covers default-value synchronization between code and config;
│      5-1 covers parameter registry completeness (all params declared in one place).
...
│      ├── 3-14-2: Drift check at load time — automated comparison of all parameter values
│      │           at system startup; generates a diff report if any mismatch detected
│      └── 3-14-3: Single-source enforcement — if drift detected → startup fails with diff;
│                  operator must designate ONE authoritative source and sync the other
│
...
│  │               (c) new value passes range check; only then apply the hot-reload
│  │
...
│  │   Human display interface. MUST expose session ID, collaboration depth, working memory
│  │   health band, and governance overhead ratio simultaneously in a single human-readable view.
...
│  │   │         governance overhead ratio (from 5-11-1)
...
│  │   When `hot_reload_enabled=1`, governance parameters MAY be updated mid-session without
│  │   stopping nodes. This adds flexibility but requires strict pre-conditions and audit
│  │   boundaries to preserve governance integrity.
│  │   Example: `collaboration_depth` lowered from 10 to 5 mid-session because budget is nearly
│  │   exhausted — this is a hot-reload event, not a node restart.
│  │   ├── 5-10-1: Hot-reload trigger — parameter change request arrives as formal governance
│  │   │           event (type: PARAM_HOT_RELOAD) routed through consensus pipeline (2-2)
│  │   ├── 5-10-2: State consistency pre-check — before any hot-reload:
│  │   │           (a) no active consensus rounds in progress,
│  │   │           (b) no uncommitted directives held by any node,
│  │   │           (c) all nodes ACK the upcoming change via 2-2 vote
│  │   └── 5-10-3: Audit boundary marker — hot-reload event written to audit log (3-9) as
│  │               immutable state-machine boundary; subsequent decisions audited against
│  │               NEW parameter value; prior decisions audited against old value
│  │
│  └── 5-11: Governance Overhead Budget  [C|T3]  [SCALE]
│      Governance mechanisms (consensus rounds, policy gate runs, audit log writes) consume
│      compute. If governance overhead exceeds `governance_overhead_ceiling_pct` of total
│      compute, the governance meta-system itself becomes a bottleneck to productive work.
│      Example: 30% of compute spent on consensus rounds (threshold: 20%) → auto-propose
│      depth reduction.
│      ├── 5-11-1: Overhead ratio tracking — compute spent on governance events /
│      │           total compute per session; logged continuously; fed to 5-4 dashboard
│      ├── 5-11-2: Overhead alert — ratio > `governance_overhead_ceiling_pct` → WARN;
│      │           displayed in health dashboard (5-4-1)
│      └── 5-11-3: Overhead reduction protocol — ratio exceeds ceiling for 3+ consecutive
│                  sessions: (a) auto-propose `collaboration_depth` reduction via 2-2 consensus;
│                  (b) suggest deferring non-critical governance events;
│                  (c) human ACK required before depth change takes effect
│
└─ Cat 6: Product Delivery & Validation  [5%]
   ...
   │   Human MUST explicitly verify the Primary Success Scenario before closure.
...
| 0-5 | Non-Functional Intent | B | T2 | Session initiation artifact contains 0-5-1/2/3 fields (scalability, flexibility, maintainability constraints) | Any multi-node session started without 0-5 fields present |
...
| 4-3 | Infra Abstraction | C | T3 | IPC health endpoint returns OK; node registry parses as valid; transport capacity adjustable without restart | IPC unreachable; registry malformed; transport requires restart to scale |
...
| 4-10 | Artifact Scale | C | T3 | When artifact count > `artifact_scale_threshold`: localized scope applied; override logged | Scale trigger reached but full-scope applied without override log |
| 4-11 | Doc Freshness | C | T3 | 100% governance docs have review event within `doc_freshness_interval` days; zero orphaned refs | Any doc overdue; orphaned reference detected in governance doc |
...
| 2-2 | Consensus | C | T3 | 100% cross-agent decisions have Propose→Vote→Finalize event sequence in log; active quorum ≤ `max_nodes_per_consensus` | Any decision without complete state-machine trace; quorum exceeded cap |
...
| 2-8 | Role Mgmt | C | T3 | 100% directives specify intended_role; Role-Instance join table current; hot-swap events logged as state-machine boundaries | Directive with node_id instead of role_id; hot-swap without boundary log |
...
| 3-13 | Governance Debt | C | T3 | GD Score computed daily; GD Score < `governance_debt_ceiling`; no new tasks halted | GD Score ≥ ceiling for >24h; new task started while ceiling breached |
| 3-14 | Config Drift | C | T3 | System starts with zero parameter drift (code default = config file value for all params) | Any parameter drift detected at load time; system starts despite drift |

### Cat 5: Operations & Control

| ID | Item | Type | Tier | PASS Condition | FAIL Trigger |
|:---|:-----|:----:|:----:|:---------------|:-------------|
| 5-1 | Param Registry | C | T3 | 0 hardcoded thresholds in code; all 36 parameters in registry | Any threshold found hardcoded outside registry |
| 5-2 | Param Validation | C | T3 | System starts successfully with valid registry; invalid registry causes exit non-zero; hot-reload pre-conditions checked | System starts with out-of-range value; hot-reload without pre-condition check |
...
| 5-4 | Dashboard | C | T3 | Dashboard displays session ID + depth + memory health band + overhead ratio simultaneously | Any required field missing from dashboard |
...
| 5-10 | Runtime Reconfig | C | T3 | When enabled: 100% hot-reloads pass state-consistency pre-check; all hot-reload events in audit log as boundary markers | Hot-reload applied without pre-check; boundary marker absent in audit log |
| 5-11 | Gov. Overhead | C | T3 | Overhead ratio ≤ `governance_overhead_ceiling_pct`; ratio logged continuously | Overhead ratio exceeds ceiling for 3+ consecutive sessions without corrective action |
...
| T11 | Node scalability | Consensus complexity | `max_nodes_per_consensus` | KPI: MTC (Mean Time to Consensus) vs. active node count; MTC increasing exponentially → scalability failing. Δ MTC per additional node is the measurement. |
...
| T25 | Execution velocity (more bypasses allowed) | Governance debt accumulation | `governance_debt_ceiling` | Lower ceiling = stricter enforcement; 0 = zero tolerance; ceiling breach → HALT |
| T26 | Hot-reload flexibility (mid-session param changes) | Audit stability (clean audit boundary) | `hot_reload_enabled` | 0=stable audit trail, no mid-session changes; 1=flexible but requires boundary events in 3-9 |
| T27 | Governance completeness (thorough enforcement) | Execution velocity (fast delivery) | `governance_overhead_ceiling_pct` | Lower ceiling = governance must be efficient; higher = thoroughness at compute cost. Distinct from T1: T1 governs WHETHER to invoke collaboration; T27 governs the COST of governance once running. |
...
| Quorum Cap | Active quorum size vs. `max_nodes_per_consensus` | Quorum ≤ cap at all times |
...
| C10 | Runtime flexibility vs. immutable audit: hot-reloading a parameter mid-session changes the rules governing active decisions, making it ambiguous which rule version applies to historical decisions | Cat 5-10 vs 3-9 | Hot-reload events written to 3-9 as immutable state-machine boundary markers. Decisions before the marker are audited against old values; decisions after against new values. Time-stamped boundary resolves ambiguity. |
| C11 | Sub-team scalability vs. global working memory: as sub-teams scale horizontally and synthesize output concurrently, their combined output can instantly saturate the global working memory threshold (1-1) | Cat 2-7 vs 1-1 | Memory compaction (1-3-2) MUST be invoked dynamically during sub-team synthesis before merged output enters global working memory. Sub-team output must be bounded by remaining memory budget. |

---

## §6. Governance Parameter Catalogue (36 parameters, 38 total config keys)

> Config key count: `_param_sections` (1) + 36 params + `last_review_ts` (1) = 38 total keys.
> Validation: load-time check that key count = 38 and all sections are consistent.
...
| `max_nodes_per_consensus` | int | 2–Count(ACTIVE_NODES) | 10 | Hard cap on active quorum size; prevents exponential MTC decay as node count grows; lower = faster consensus but reduced representation | general | T11 |
...
| `governance_debt_ceiling` | int | 0–100 GD points | 10 | Max GD Score (see §1.2 Governance Debt definition) before new tasks are halted. GD Score = (bypass×5) + (stale_PM×3) + (floor(degraded_hours/24)×2). Lower = stricter enforcement. | cat3 | T25 |
...
| `artifact_scale_threshold` | int | 100–100000 | 5000 | Artifact count above which localized dependency scope is applied (4-10). Below threshold: full-repo scope. Above: localized subgraph scope. | cat4 | — |
| `doc_freshness_interval` | int | 7–365 days | 30 | Max days before a governance document requires a review event (4-11). Documents older than this interval trigger WARN escalation. | cat4 | T6 |
...
| `hot_reload_enabled` | bool | 0/1 | 0 | Permits parameter hot-reload without node restart (5-10). 0=safe default; changes require node restart. 1=hot-reload allowed under 5-10 pre-conditions. | cat5 | T26,C10 |
| `governance_overhead_ceiling_pct` | int | 5–50 % | 20 | Max % of total session compute allowed for governance meta-system events (consensus, policy checks, audit writes). Exceeding ceiling for 3+ sessions triggers reduction protocol (5-11-3). | cat5 | T27 |
...
> Category weights are kept constant across v3–v8 to preserve historical Root Score comparability.

### Item Counts by Category (v8)

```
Cat 0:  5 items  (was 4; +1: 0-5)
Cat 1:  5 items  (unchanged)
Cat 2:  9 items  (unchanged; 2-8-5 is sub-item, not top-level)
Cat 3: 14 items  (was 12; +2: 3-13, 3-14)
Cat 4: 11 items  (was 9; +2: 4-10, 4-11)
Cat 5: 11 items  (was 9; +2: 5-10, 5-11)
Cat 6:  3 items  (unchanged)
Total: 58 items  (was 51)
```
...
| G39 | Non-functional intent capture schema | 0-5 | 🔶 NEW — 0-5-1/2/3 field schema in session initiation artifact |
| G40 | Governance Debt + Configuration Drift | 3-13, 3-14 | 🔶 NEW — GD Score computation + drift check at load time |
| G41 | Artifact Scale + Doc Freshness | 4-10, 4-11 | 🔶 NEW — scale trigger detection + freshness scheduler |
| G42 | Runtime Reconfiguration + Overhead Budget | 5-10, 5-11 | 🔶 NEW — hot-reload protocol + overhead ratio tracking |
...
| Pri | Task | Item | Notes |
|:---:|:-----|:-----|:------|
| **P0** | Deploy parameter registry (36 params, 38 keys) | 5-1 | Foundation for all other work |
...
| **P1** | Non-functional intent fields (0-5-1/2/3) in session initiation schema | 0-5 | G39 |
...
| **P3** | Governance Document Freshness scheduler | 4-11 | G41 |
...
| G | Gap Analysis | Cat 0–6 | — | MECE completeness identification, including F/S/M quality attribute coverage |
| H | Health Check | Cat 5 | Cat 1 | Working memory, system health, governance overhead ratio (5-11-1) |
| I | Integration Test | Cat 3, 4 | Cat 2 | Cross-component testing |
| J | Policy Gate | Cat 3 (static) | Cat 5 | N-check policy regression (Judge); includes config drift check (3-14) |
...
> **Quality Attribute note**: F/S/M quality attribute completeness review is triggered by Axis G
> on any framework update. Axis H covers SCALE and MAINT observability. Axis J covers MAINT
> static checks including configuration drift detection (3-14).
...
> Items marked [TODO] are defined in v8 but not yet implemented in this reference system.
...
> Target config.json keys for v8: 38 (current: 24; P0 task expands to 38)
...
# Parameter registry key count (Cat 5-1) [target v8: 38]
...
# [TODO v8] Non-functional intent fields (Cat 0-5)
# Select-String -Path ".ai\sessions\room-*\handoff.md" -Pattern "SCALE_CONSTRAINTS:|FLEX_CONSTRAINTS:|MAINT_CONSTRAINTS:"

# [TODO v8] Governance Debt Score (Cat 3-13)
# python .\_sys\checks\check_governance_debt.py

# [TODO v8] Configuration Drift check (Cat 3-14)
# python .\_sys\checks\check_drift.py

# [TODO v8] Artifact scale count (Cat 4-10)
# python .\_sys\checks\check_artifact_scale.py

# [TODO v8] Governance doc freshness (Cat 4-11)
# python .\_sys\checks\check_doc_freshness.py

# [TODO v8] Governance overhead ratio (Cat 5-11)
# python .\_sys\checks\check_overhead.py

# [TODO v8] Participation mode validation
# python .\_sys\checks\check_modes.py

# [TODO v8] Role-Instance join table validation
# python .\_sys\checks\check_join_table.py

# [TODO v8] Functional verification gate (run tests)
# python -m pytest tests/ --tb=short

# [TODO v8] Remediation velocity report
# python .\_sys\checks\remediation_velocity.py
```

## A.4 Current Implementation State (CC+GC, 2026-06-10)

> New items (0-5, 3-13, 3-14, 4-10, 4-11, 5-10, 5-11) start at T0 (Current Tier = 0).
> This temporarily lowers the Root Score vs. v7 (44.2% → 40.1%) until G39–G42 are implemented.

| Cat | Name | Items | Weight | Score | Target | Δ |
|:---:|:-----|:-----:|:------:|:-----:|:------:|:--:|
| 0 | Human Intent | 5 | 8% | 0.20 | 0.95 | +0.75 |
| 1 | Cognitive Continuity | 5 | 17% | 0.60 | 0.95 | +0.35 |
| 2 | Collaboration | 9 | 22% | 0.28 | 0.95 | +0.67 |
| 3 | System Integrity | 14 | 28% | 0.45 | 0.95 | +0.50 |
| 4 | Environment | 11 | 13% | 0.46 | 0.95 | +0.49 |
| 5 | Operations | 11 | 7% | 0.20 | 0.95 | +0.75 |
| 6 | Delivery | 3 | 5% | 0.40 | 0.95 | +0.55 |
| **Root** | | **58** | **100%** | **~0.40 (40.1%)** | **~0.95** | **+0.55** |

```
Root = 0.08×0.20 + 0.17×0.60 + 0.22×0.28 + 0.28×0.45 + 0.13×0.46 + 0.07×0.20 + 0.05×0.40
     = 0.016 + 0.102 + 0.062 + 0.127 + 0.060 + 0.014 + 0.020
     = 0.401 (40.1%)

Δ from v7 (44.2%): -4.1% — caused by 7 new T0 items added to framework.
This is expected: framework is MORE complete (58 items vs. 51), but implementation has not caught up.

G39-G42 implementation → Root ~47% (+7.0%)
P0+P1 implementation   → Root ~54%
P0+P2 implementation   → Root ~67%
...
| TAXONOMY_v7.md | `.\_sys\docs\TAXONOMY_v7.md` | Superseded version (READ-ONLY) |
| TAXONOMY_v8.md | `.\_sys\docs\TAXONOMY_v8.md` | This document |
...
- TAXONOMY_v7.md (READ-ONLY): `P:\_sys\docs\TAXONOMY_v7.md`
  Source document. v8 supersedes v7. All §0–§10 universal content evolved from v7.
  Key sections cited: §1–§10, Appendix A, Appendix B (structures preserved in v8).

- R=10 Collaboration Log (CC+GC, 2026-06-10):
  Round 1: Gemini identified Q1–Q15 (framing, FLEX/SCALE/MAINT gaps, new items, parameters).
  Round 2: Claude refinements (4-10 rename, GD formula, compromise on tagging vs. sub-items,
  addition of 4-11/3-14/5-11/T27). Unanimous consensus achieved before file creation.
